...
export class FagsakResolver extends Component<OwnProps> {
  ...
  componentDidUpdate = prevProps => {
    ...
    const hasBehandlingVersjonChanged =
      prevProps.behandlingVersjon && prevProps.behandlingVersjon !== behandlingVersjon;
  ...
  resolveFagsak() {
    const { selectedSaksnummer, fetchFagsak } = this.props;

    fetchFagsak({ saksnummer: selectedSaksnummer }, { keepData: true });
  }

  render() {
    const { fetchFagsakPending, fagsakResolved, children, selectedFagsak } = this.props;
    if (!fagsakResolved) {
...
const mapStateToProps = state => ({
  ...
  fetchFagsakPending:
    !fpsakApi.FETCH_FAGSAK.getRestApiFinished()(state) || !fpsakApi.FETCH_FAGSAK.getRestApiError()(state),
...
export default withRouter(
  connect(mapStateToProps, mapDispatchToProps)(requireProps(['selectedSaksnummer'], <LoadingPanel />)(FagsakResolver)),
);
