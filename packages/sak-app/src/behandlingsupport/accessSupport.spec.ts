...
describe('accessSupport', () => {
  ...
  describe('godkjenningsFaneAccess', () => {
    ...
    const validFagsakStatuserKlage = [
    ...
    it('saksbehandler med beslutter-rolle skal ha tilgang til godkjenningsfanen', () => {
      const accessForSaksbehandler = godkjenningsFaneAccess(
    ...
    it('veileder skal ikke ha aktivert tilgang til godkjenningsfanen', () => {
      const accessForVeileder = godkjenningsFaneAccess(
        veilederAnsatt,
        validFagsakStatus,
        validBehandlingStatus,
        ansvarligSaksbehandler,
      ...
      const accessForVeilederOverstyrer = godkjenningsFaneAccess(
        veilederOgBeslutterAnsatt,
        validFagsakStatus,
        validBehandlingStatus,
        ansvarligSaksbehandler,
        behandlingType,
    ...
    it('saksbehandler med beslutter-rolle skal ha tilgang til godkjenningsfanen hvis hen er ansvarlig saksbehandler på behandlingen', () => {
      ...
      const ansvarligSaksbehandlerBeslutter = 'beslutter';
      const accessForSaksbehandler = godkjenningsFaneAccess(
        beslutterAnsatt,
        validFagsakStatus,
        validBehandlingStatus,
        ansvarligSaksbehandlerBeslutter,
        behandlingType,
      );
    ...
    forEachFagsakAndBehandlingStatus((fagsakStatus, behandlingStatus) => {
      ...
      it(getTestName('tilgang til å godkjenne klage', expected, fagsakStatus, behandlingStatus), () => {
        const access = godkjenningsFaneAccess(
          saksbehandlerOgBeslutterAnsatt,
          { kode: fagsakStatus, kodeverk: 'BEHANDLING_STATUS' },
          { kode: behandlingStatus, kodeverk: 'BEHANDLING_STATUS' },
          null,
          { kode: BehandlingType.KLAGE, kodeverk: 'BEHANDLING_STATUS' },
        );
  ...
  describe('sendMeldingAccessSelector', () => {
    describe('saksbehandler og veileder', () => {
      ...
      it('saksbehandler skal ha tilgang til å sende melding', () => {
        const accessForSaksbehandler = sendMeldingAccess(
          saksbehandlerAnsatt,
          validFagsakStatus,
          validBehandlingStatus,
          behandlingType,
      ...
      it('veileder skal ikke ha aktivert tilgang til å sende melding', () => {
        const accessForVeileder = sendMeldingAccess(
          veilederAnsatt,
          validFagsakStatus,
          validBehandlingStatus,
          behandlingType,
      ...
      forEachFagsakAndBehandlingStatus((fagsakStatus, behandlingStatus) => {
        ...
        it(getTestName('tilgang til å sende melding', expected, fagsakStatus, behandlingStatus), () => {
          const access = sendMeldingAccess(
            ...
            { kode: behandlingStatus, kodeverk: 'FAGSAK_STATUS' },
            behandlingType,
          );
