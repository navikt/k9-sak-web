...
import { NavAnsatt, Fagsak } from '@k9-sak-web/types';
...
const getReturnedIsRelevant = (isOnHold, toTrinnsAksjonspunkter: TotrinnskontrollAksjonspunkt[] = [], status) =>
  !isOnHold &&
...
export const getAccessibleSupportPanels = (returnIsRelevant, approvalIsRelevant, rettigheter) =>
...
export const getEnabledSupportPanels = (accessibleSupportPanels, sendMessageIsRelevant, rettigheter) =>
...
const BehandlingSupportIndex: FunctionComponent<OwnProps> = ({
  ...
  const { data: totrinnArsaker } = restApiHooks.useRestApi<TotrinnskontrollAksjonspunkt[]>(
    FpsakApiKeys.TOTRINNSAKSJONSPUNKT_ARSAKER,
    undefined,
    {
  ...
  const { data: totrinnArsakerReadOnly } = restApiHooks.useRestApi<TotrinnskontrollAksjonspunkt[]>(
    FpsakApiKeys.TOTRINNSAKSJONSPUNKT_ARSAKER_READONLY,
    undefined,
    {
      updateTriggers: [behandlingId, behandlingStatusKode],
  ...
  const rettigheter = useMemo(
    ...
    [fagsak.status, behandling?.id, behandling?.versjon],
  );
  const returnedIsRelevant = useMemo(
    () => getReturnedIsRelevant(erPaVent, totrinnArsakerReadOnly, behandling?.status),
  ...
  const acccessibleSupportPanels = useMemo(
    () => getAccessibleSupportPanels(returnedIsRelevant, approvalIsRelevant, rettigheter),
    [returnedIsRelevant, approvalIsRelevant, rettigheter],
  );
  const sendMessageIsRelevant = useMemo(() => fagsak && !erPaVent, [fagsak, erPaVent]);
  const enabledSupportPanels = useMemo(
    () => getEnabledSupportPanels(acccessibleSupportPanels, sendMessageIsRelevant, rettigheter),
