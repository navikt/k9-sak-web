...
const formatErrorMsgHeader = (systemNavn, nedeFremTilTidspunkt) =>
  nedeFremTilTidspunkt ? (
    <FormattedMessage
      id="IntegrationStatusPanel.DowntimeUntil"
      values={{
        system: systemNavn,
        datetime: `${dateFormat(nedeFremTilTidspunkt)} ${timeFormat(nedeFremTilTidspunkt)}`,
      }}
    />
  ) : (
...
class IntegrationStatusPanel extends Component<OwnProps, OwnState> {
  ...
  render() {
    const { integrationStatusList } = this.props;
    const { toggleOpen } = this.state;
      ...
      >
        ...
        <Row className={styles.row}>
          ...
          <Column xs="9" className={styles.text}>
            <Undertittel>
              <FormattedMessage id="IntegrationStatusPanel.ServiceDowntime" />
            </Undertittel>
          </Column>
        </Row>
        ...
        <Row className={styles.rowContents}>
          ...
          <Column xs="11">
            {integrationStatusList.map(isl => {
                ...
                return (
                  <div key={isl.systemNavn}>
                    ...
                    <div className={styles.details}>
                      <Undertekst>
                        <FormattedMessage id="IntegrationStatusPanel.Endpoint" />
                      </Undertekst>
                      <Normaltekst>{isl.endepunkt}</Normaltekst>
                      <VerticalSpacer fourPx />
                      <Undertekst>
                        <FormattedMessage id="IntegrationStatusPanel.ErrorMessage" />
                      </Undertekst>
                      <Normaltekst>{isl.feilmelding}</Normaltekst>
                      <VerticalSpacer fourPx />
                      <Undertekst>
                        <FormattedMessage id="IntegrationStatusPanel.Stacktrace" />
                      </Undertekst>
              ...
              return (
                <Normaltekst key={isl.systemNavn}>
                  {formatErrorMsgHeader(isl.systemNavn, isl.nedeFremTilTidspunkt)}
                </Normaltekst>
              );
            })}
          </Column>
