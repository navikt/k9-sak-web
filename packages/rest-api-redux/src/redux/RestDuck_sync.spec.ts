...
describe('RestDuck (sync)', () => {
  ...
  it('skal sette flagg for started og finished og legge ressurs i state', () => {
    ...
    const params = { id: 'id' };
    return store.dispatch(getRessursDuck.actionCreators.execRequest(params)).then(() => {
  ...
  it('skal sette flagg for started og og legge feil i state', () => {
    mockAxios.onGet(ressursEndpointIncludingContextPath).reply(404, 'Resource not found');
    const store = createStore();
    ...
    const params = { id: 'id' };
    return store.dispatch(getRessursDuck.actionCreators.execRequest(params)).catch(() => {
  ...
  it('skal beholde data ved ny request hvis "keepData"-flagget er satt', () => {
    ...
    const params = { id: 'id' };
    return store.dispatch(getRessursDuck.actionCreators.execRequest(params, { keepData: true })).then(() => {
      const [requestStartedAction] = store.getActions();
      expect(requestStartedAction.type).to.eql(getRessursDuck.actionTypes.requestStarted());
  ...
  it('selectors skal hente ut state', () => {
    ...
    return store.dispatch(getRessursDuck.actionCreators.execRequest(params)).then(() => {
      ...
      const stateAfterRequestFinished = {
        dataContext: {
          ressurs: getRessursDuck.reducer(
            getRessursDuck.stateSelector(stateAfterRequestStarted),
