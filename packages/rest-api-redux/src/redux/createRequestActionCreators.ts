...
import { NotificationMapper, RequestRunner } from '@fpsak-frontend/rest-api-old';
...
const createRequestThunk = (requestRunner, actionCreators, reduxEvents, resultKeyActionCreators) => (
  params: any,
  options: Options = {},
) => (dispatch: Dispatch) => {
  ...
  if (resultKeyActionCreators) {
    notificationMapper.addRequestStartedEventHandler(() =>
      dispatch(resultKeyActionCreators.requestStarted(params, options)),
    );
  ...
  if (reduxEvents.getErrorMessageActionCreator()) {
    notificationMapper.addRequestErrorEventHandler((data, type) =>
      dispatch(reduxEvents.getErrorMessageActionCreator()({ ...data, type })),
    );
    notificationMapper.addHaltedOrDelayedEventHandler((data, type) =>
      dispatch(reduxEvents.getErrorMessageActionCreator()({ ...data, type })),
    );
    notificationMapper.addPollingTimeoutEventHandler((data, type) =>
      dispatch(reduxEvents.getErrorMessageActionCreator()({ ...data, type })),
    );
  }

  if (reduxEvents.getPollingMessageActionCreator()) {
    notificationMapper.addUpdatePollingMessageEventHandler(data =>
      dispatch(reduxEvents.getPollingMessageActionCreator()(data)),
    );
    notificationMapper.addRequestFinishedEventHandler(
      (_data, _type, isAsync: boolean) => isAsync && dispatch(reduxEvents.getPollingMessageActionCreator()()),
    );
    notificationMapper.addRequestErrorEventHandler(
      (_data, _type, isAsync: boolean) => isAsync && dispatch(reduxEvents.getPollingMessageActionCreator()()),
    );
  }
