...
const getActionTypes = (requestRunner: RequestRunner) => ({
  requestStarted: () =>
    createActionType(getName(requestRunner), 'STARTED', getRestMethod(requestRunner), getPath(requestRunner)),
  requestFinished: () =>
    createActionType(getName(requestRunner), 'FINISHED', getRestMethod(requestRunner), getPath(requestRunner)),
  requestError: () =>
    createActionType(getName(requestRunner), 'ERROR', getRestMethod(requestRunner), getPath(requestRunner)),
  reset: () => createActionTypeWithoutPath(getName(requestRunner), 'RESET'),
  updatePollingMessage: () =>
    createActionType(
      getName(requestRunner),
      'POLLING_MESSAGE_RECEIVED',
      getRestMethod(requestRunner),
      getPath(requestRunner),
    ),
  statusRequestStarted: () =>
    createActionType(getName(requestRunner), 'STATUS_STARTED', getRestMethod(requestRunner), getPath(requestRunner)),
  statusRequestFinished: () =>
    createActionType(getName(requestRunner), 'STATUS_FINISHED', getRestMethod(requestRunner), getPath(requestRunner)),
  pollingTimeout: () =>
    createActionType(getName(requestRunner), 'POLLING_TIMEOUT', getRestMethod(requestRunner), getPath(requestRunner)),
