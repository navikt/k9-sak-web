...
export class DataFetcherTriggers {
  ...
  shouldFetch = (previousTriggerValues, cacheParams) => {
    ...
    if (
      ...
      Object.keys(this.triggers).some(
        key => previousTriggerValues[key] !== undefined && previousTriggerValues[key] !== this.triggers[key],
      )
    ) {
      return true;
    }
    ...
    if (
      !this.isRequiredForInitiallFetching &&
      cacheParams &&
      Object.keys(this.triggers).every(key => !cacheParams[key] && !cacheParams[key])
    ) {
      return false;
    }
  ...
  hasChanged = previousTriggerValues =>
    previousTriggerValues && Object.keys(this.triggers).some(key => previousTriggerValues[key] !== this.triggers[key]);
}
...
const DataFetcher: FunctionComponent<OwnProps> = ({
  ...
  const activeEndpointsData = endpoints
    .filter(e => e.isEndpointEnabled())
    .map(e => ({
      endpoint: e,
      ...endpointData[e.name],
    }));
  ...
  useEffect(() => {
    ...
    const endpointsToFetchFrom = activeEndpointsData.filter(e =>
      fetchingTriggers.shouldFetch(ref.current, e.endpoint.getRestApiCacheParams()(state)),
    );
    ...
    Promise.all(
      ...
      if (cancel) {
        ...
        console.warn(
          `DataFetcher som henter data fra endepunktene ${endpoints
            .map(e => e.name)
            .join(', ')} har kjørt unmount før data er ferdighentet. Dette kan være feil!`,
        );
        return;
      }
      setFetchingData({
        hasFinishedFetching: true,
        data: endpointsToFetchFrom.reduce(
          (acc, endpoint, index) => ({
            ...acc,
