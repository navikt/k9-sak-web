...
const pollingMessageReducer = (state = initialState, action: Action = { type: '' }) => {
  // NOSONAR Switch brukes som standard i reducers
  switch (
    action.type // NOSONAR Switch brukes som standard i reducers
  ) {
...
export const getRequestPollingMessage = createSelector(
  [getAppContext],
  appContext => appContext.restRequestPollingMessage,
);
