...
export class Vurderingsoversikt {
  ...
  constructor(data: Partial<Vurderingsoversikt>) {
    try {
      ...
      this.resterendeVurderingsperioder = data.resterendeVurderingsperioder.map(({ fom, tom }) => new Period(fom, tom));
      this.resterendeValgfrieVurderingsperioder = data.resterendeValgfrieVurderingsperioder.map(
        ({ fom, tom }) => new Period(fom, tom),
      );
      this.søknadsperioderTilBehandling = data.søknadsperioderTilBehandling.map(({ fom, tom }) => new Period(fom, tom));
