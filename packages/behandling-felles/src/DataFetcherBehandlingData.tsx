...
const DataFetcherBehandlingData: FunctionComponent<OwnProps> = ({
  ...
  const data = activeEndpoints.reduce(
    (acc, e) => ({
      ...acc,
  ...
  useEffect(() => {
    ...
    const endpointsToFetchFrom = activeEndpoints.filter(
      e => !behandlingDataCache.hasLatestDataFor(endpointData[e.name].formattedName),
    );
    endpointsToFetchFrom.forEach(e => {
      behandlingDataCache.startFetch(behandlingVersion, endpointData[e.name].formattedName);
    });
    ...
    Promise.all(
      ...
      .then(responses => {
        responses.forEach((r, index) =>
          behandlingDataCache.setData(
            behandlingVersion,
            endpointData[endpointsToFetchFrom[index].name].formattedName,
            r.payload,
          ),
        );
      })
  ...
  if (
    !hasFailed &&
    isFetchFinished &&
    (behandlingVersion === ref.current ||
      behandlingDataCache.hasLatestDataForAll(activeEndpoints.map(e => endpointData[e.name].formattedName)))
  ) {
    return render({
