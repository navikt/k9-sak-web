...
import { Behandling, NavAnsatt, Aksjonspunkt } from '@k9-sak-web/types';
...
export const utledFaktaPaneler = (
  ...
  return faktaPanelDefinisjoner
    .filter(
      panel => panel.aksjonspunkterCodes.some(a => apCodes.includes(a)) || panel.showComponent(utvidetEkstraPanelData),
    )
    .map(panel => {
      const filtrerteAksjonspunkter = aksjonspunkter.filter(ap =>
        panel.aksjonspunkterCodes.includes(ap.definisjon.kode),
      );
      const harApneAksjonspunkter = filtrerteAksjonspunkter.some(
        ap => isAksjonspunktOpen(ap.status.kode) && ap.kanLoses,
      );
      return {
...
): FaktaPanelFaktaPanelMenyRadMeny[] =>
