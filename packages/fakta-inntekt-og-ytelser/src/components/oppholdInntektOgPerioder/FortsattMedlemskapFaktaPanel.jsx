...
import { dateAfterOrEqual, DDMMYYYY_DATE_FORMAT, hasValidDate, ISO_DATE_FORMAT, required } from '@fpsak-frontend/utils';
 ...
 */
export const FortsattMedlemskapFaktaPanelImpl = ({ readOnly, skjaringstidspunkt, changedOpplysninger }) => (
  <>
    ...
    <Row>
      ...
      <Column xs="6">
        {changedOpplysninger.length > 0 && (
          <Normaltekst>
            <FormattedMessage id="FortsattMedlemskapFaktaPanel.OpplysningerFraFolkeregisteret" />
          </Normaltekst>
        )}
        {changedOpplysninger.map(opplysning => {
          if (opplysning.endretAttributt === 'Personstatus') {
            return (
              <Normaltekst key={1}>
                <FormattedMessage id="FortsattMedlemskapFaktaPanel.EndretPersonstatus" values={getParam(opplysning)} />
              </Normaltekst>
            );
          }
          if (opplysning.endretAttributt === 'StatsborgerskapRegion') {
            return (
              <Normaltekst key={2}>
                <FormattedMessage
                  id="FortsattMedlemskapFaktaPanel.EndretStatsborgerskap"
                  values={getParam(opplysning)}
                />
              </Normaltekst>
            );
          }
          if (opplysning.endretAttributt === 'Adresse') {
            return (
              <Normaltekst key={3}>
                <FormattedMessage id="FortsattMedlemskapFaktaPanel.NyUtlandsadresse" values={getParam(opplysning)} />
              </Normaltekst>
            );
          }
          return null;
