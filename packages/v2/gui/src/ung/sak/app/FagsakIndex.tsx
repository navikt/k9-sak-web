...
const FagsakIndex = ({ ungSakBackendClient }: FagsakIndexProps) => {
  ...
  const { data: alleKodeverkUngSak } = useQuery({
    queryKey: ['ungKodeverk'],
    queryFn: () => ungSakBackendClient.getAlleKodeverk(),
  });

  const erBehandlingEndretFraUndefined =
    behandlingId && behandlingVersjon && useBehandlingEndret(behandlingId, behandlingVersjon);

  const { data: fagsakPerson } = useQuery({
    queryKey: ['fagsakPerson', selectedSaksnummer],
    queryFn: () => ungSakBackendClient.getFagsakPerson(selectedSaksnummer),
    enabled: !!selectedSaksnummer,
  });

  const { data: fagsak } = useQuery({
    queryKey: ['fagsak', selectedSaksnummer, behandlingId, behandlingVersjon],
    queryFn: () => ungSakBackendClient.getFagsak(selectedSaksnummer),
    enabled: !!selectedSaksnummer && !erBehandlingEndretFraUndefined,
    // keepData: true <-- var slik i gammel kode. Hva gjør vi med denne?
  });
