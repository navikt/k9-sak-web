...
export const MedlemskapReferanse: Story = {
  args: {
    children: '§ 9-2 jf. kapittel 2',
  },
  play: async ({ canvasElement, step }) => {
    const { linkEls, root } = elemsfinder(canvasElement);
    await step('Lenketeksten for kapittel inkluderer hele "Kapittel X"-frasen, ikke bare tallet', async () => {
      await expect(linkEls()).toHaveLength(2);
      const [paragrafLink, kapittelLink] = linkEls();
