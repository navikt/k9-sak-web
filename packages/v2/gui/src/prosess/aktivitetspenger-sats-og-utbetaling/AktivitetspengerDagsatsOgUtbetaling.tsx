...
import type { GetSatsOgUtbetalingPerioderAktivitetspengerResponse } from '@k9-sak-web/backend/ungsak/generated/types.js';
import { ung_sak_kontrakt_ungdomsytelse_ytelse_UtbetalingStatus as UtbetalingStatus } from '@k9-sak-web/backend/ungsak/generated/types.js';
import type { AktivitetspengerSatsOgUtbetalingBackendApiType } from './AktivitetspengerSatsOgUtbetalingBackendApiType.js';
import { AktivitetspengerBeregningsDetaljer } from './AktivitetspengerBeregningsDetaljer.js';
import styles from './aktivitetspengerSatsOgUtbetaling.module.css';
import { formatMonthYear, formatSats } from './aktivitetspengerSatsUtils.js';
import { DataCellWithValue } from './DataCellWithValue.js';
...
const StatusTag = ({ status }: { status: string }) => (
  <Tag size="small" variant={status === UtbetalingStatus.TIL_UTBETALING ? 'info' : 'success'}>
    {status === UtbetalingStatus.TIL_UTBETALING ? 'Til utbetaling' : 'Utbetalt'}
  </Tag>
);

const sortSatser = (data: GetSatsOgUtbetalingPerioderAktivitetspengerResponse) =>
  data?.toSorted((a, b) => new Date(a.måned).getTime() - new Date(b.måned).getTime()).toReversed();

interface AktivitetspengerDagsatsOgUtbetalingProps {
  api: AktivitetspengerSatsOgUtbetalingBackendApiType;
  behandling: { uuid: string };
}

export const AktivitetspengerDagsatsOgUtbetaling = ({ api, behandling }: AktivitetspengerDagsatsOgUtbetalingProps) => {
  const {
    data: satser,
    isLoading: satserIsLoading,
    isError: satserIsError,
  } = useQuery({
    queryKey: ['avp-satser', behandling.uuid],
                          ...
                          return (
                            <>
                              <Table.ExpandableRow
                                ...
                                content={
                                  <AktivitetspengerBeregningsDetaljer
