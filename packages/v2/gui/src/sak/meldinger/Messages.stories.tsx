...
export const DefaultStory: Story = {
  beforeEach: () => {
    api.setDelayMillis(600);
    return () => api.setDelayMillis(0);
  },
  args: {
    fagsak: {
      saksnummer: '100',
...
export const TilTredjepartsmottaker: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    const { sendTilTredjepartEl, orgnrInp, orgNavnInp, sendBrevBtn, fritekstEl } = elemsfinder(canvasElement);
    ...
    await step('Inntasting av org nr skal fungere', async () => {
      ...
      await step('Bestilling skal ha tredjepartsmottaker satt', async () => {
        await userEvent.type(fritekstEl(), 'Brevinnhold');
        await userEvent.click(sendBrevBtn());
        await expect(api.sisteBestilling?.overstyrtMottaker?.id).toEqual(orgnr);
      });
    });
  },
};

export const BestillingTest: Story = {
  args: {
    ...DefaultStory.args,
  },
  play: async ({ canvasElement, args }) => {
    const { fritekstEl, sendBrevBtn } = elemsfinder(canvasElement);
    const testText = 'Brevtekst innhold';
    await userEvent.type(fritekstEl(), testText);
    await userEvent.click(sendBrevBtn());
    const expectedBestilling: BestillBrevDto = {
      behandlingId: args.behandling.id,
      brevmalkode: args.maler[0]?.kode || '',
      fritekst: testText,
      fritekstbrev: undefined,
      overstyrtMottaker: templates[0]?.mottakere[0],
    };
    await expect(api.sisteBestilling).toEqual(expectedBestilling);
    expect(fritekstEl()).toHaveValue('');
  },
};
