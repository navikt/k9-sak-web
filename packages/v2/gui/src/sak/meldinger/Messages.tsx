...
const Messages = ({maler, sakstype, eksternReferanse, personopplysninger, arbeidsgiverOpplysningerPerId, api}: MessagesProps) => {
  ...
  const MalSelect = () =>
    <Select label="Mal" size="small" value={valgtMalkode} onChange={(e) => setValgtMalkode(e.target.value)}>
      {
        maler.map(mal => <option key={mal.kode} value={mal.kode}>{mal.navn}</option>)
      }
    </Select>
      ...
      return <Select
        ...
        placeholder="Velg type">
        {
          fritekstForslag.map(forslag => {
            return <option key={forslag.tittel} value={forslag.tittel}>{forslag.tittel}</option>
          })
        }
      </Select>
    }
  ...
  const MottakerSelect = () => {
    if((valgtMal?.mottakere?.length ?? 0) > 0) {
      return <Select
        label="Mottaker" size="small" placeholder="Velg mottaker" value={valgtMottakerId}
        onChange={e => setValgtMottakerId(e.target.value)}>
        {
          valgtMal?.mottakere.map(mottaker => {
            return <option key={mottaker.id} value={mottaker.id}>
              {lagVisningsnavnForMottaker(mottaker.id, personopplysninger, arbeidsgiverOpplysningerPerId)}
            </option>
          })
        }
        {
          valgtMal?.støtterTredjepartsmottaker && <option key={tredjepartsmottakerValg} value={tredjepartsmottakerValg}>Tredjepartsmottaker (skriv inn organisasjonsnr)</option>
        }
      </Select>
    }
    return null
