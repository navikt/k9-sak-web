import { Venteårsak as AksjonspunktDtoVenteårsak } from '@k9-sak-web/backend/k9sak/kodeverk/behandling/aksjonspunkt/Venteårsak.js';
...
describe('<MenySettPaVentIndex>', () => {
  it('skal vise modal og velge å åpne ta behandling av vent', async () => {
    ...
    const navigerEtterSattPåVent = vi.fn();
    ...
    render(
      ...
      >
        <MenySettPaVentIndexV2
          ...
          erKlage={false}
          navigerEtterSattPåVent={navigerEtterSattPåVent}
        />
      </KodeverkProvider>,
    );
    ...
    await fireEvent.change(venteArsakFelt, { target: { value: AksjonspunktDtoVenteårsak.MEDISINSKE_OPPLYSNINGER } });

    // Submit form
    const form = screen.getByTestId('SettPaVentModal').querySelector('form');
