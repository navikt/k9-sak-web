...
const getCommonBehandlingsprosessSelectors = (
  ...
  const getBehandlingspunkter = createSelector([getBehandlingspunkterProps], (bpProps = []) =>
    bpProps.map(p => p.code),
  );

  const getBehandlingspunkterStatus = createSelector([getBehandlingspunkterProps], bpProps =>
    arrayToObject(
      bpProps,
      bpProp => bpProp.code,
      bpProp => bpProp.status,
    ),
  );

  const getBehandlingspunkterTitleCodes = createSelector([getBehandlingspunkterProps], bpProps =>
    arrayToObject(
      bpProps,
      bpProp => bpProp.code,
      bpProp => bpProp.titleCode,
    ),
  );

  const getBehandlingspunktAksjonspunkterNames = createSelector([getBehandlingspunkterProps], bpProps =>
    arrayToObject(
      bpProps,
      bpProp => bpProp.code,
      bpProp => bpProp.apCodes,
    ),
  );

  const getBehandlingspunkterVilkar = createSelector([getBehandlingspunkterProps], bpProps =>
    arrayToObject(
      bpProps,
      bpProp => bpProp.code,
      bpProp => bpProp.vilkarene,
    ),
  );
  ...
  const getBehandlingspunktAksjonspunkter = createSelector(
    ...
    (aksjonspunkter = [], behandlingspunktAksjonspunkterNames = {}) => {
      const valueFunction = bp =>
        behandlingspunktAksjonspunkterNames[bp]
  ...
  const getBehandlingspunkterWithOpenAksjonspunkter = createSelector(
  ...
  const getVedtakBehandlingspunkt = createSelector(
    ...
    (bps = [], bpStatus = {}) =>
      bps.find(bp => bp === behandlingspunktCodes.VEDTAK && bpStatus[bp] !== vilkarUtfallType.IKKE_VURDERT),
  );
  ...
  const getSelectableBehandlingspunkter = createSelector(
    ...
    (bps = [], bpsWithOpenAps = [], bpStatus = {}, overriddenBps = []) =>
      bps.filter(
        bp =>
          !(bpStatus[bp] === vilkarUtfallType.IKKE_VURDERT && !bpsWithOpenAps.includes(bp)) ||
          overriddenBps.includes(bp),
      ),
  );
  ...
  const getSelectedBehandlingspunkt = createSelector(
    ...
    (selectableBps = [], defaultBehandlingspunkt = null, selectedBpNavn = null) =>
      selectedBpNavn === DEFAULT_BEHANDLINGSPROSESS
  ...
  const getIsSelectedBehandlingspunktOverridden = createSelector(
    ...
    (selectedBehandlingspunkt, overriddenBehandlingspunkter = []) =>
      overriddenBehandlingspunkter.includes(selectedBehandlingspunkt),
  );

  const getSelectedBehandlingspunktStatus = createSelector(
    [getSelectedBehandlingspunkt, getBehandlingspunkterStatus],
    (selectedBehandlingspunkt, behandlingspunkterStatus = {}) =>
      behandlingspunkterStatus[selectedBehandlingspunkt] ? behandlingspunkterStatus[selectedBehandlingspunkt] : '',
  );

  const getSelectedBehandlingspunktTitleCode = createSelector(
    [getSelectedBehandlingspunkt, getBehandlingspunkterTitleCodes],
    (selectedBehandlingspunkt, behandlingspunkterTitleCodes = {}) =>
  ...
  const getSelectedBehandlingspunktAksjonspunkter = createSelector(
    ...
    (selectedBehandlingspunkt, behandlingspunktAksjonspunkter = {}) =>
  ...
  const getSelectedBehandlingspunktVilkar = createSelector(
    ...
    (selectedBehandlingspunkt, behandlingspunktVilkar = {}) =>
      behandlingspunktVilkar[selectedBehandlingspunkt] ? behandlingspunktVilkar[selectedBehandlingspunkt] : [],
  );
  ...
  const getNotAcceptedByBeslutter = createSelector(
    ...
    (selectedBehandlingspunkt, aksjonspunkter = {}, merknaderFraBeslutter = {}) => {
      ...
      return (
        ap !== undefined && ap.length > 0 && !!merknaderFraBeslutter[ap[0]] && merknaderFraBeslutter[ap[0]].notAccepted
      );
    },
  );

  const findApStatus = punktAksjonspunkter =>
    punktAksjonspunkter.length > 0
      ? punktAksjonspunkter.some(ap => isAksjonspunktOpen(ap.status.kode) && ap.kanLoses)
      : false;

  const getAksjonspunkterOpenStatus = createSelector(
    [getBehandlingspunktAksjonspunkter, getOverrideBehandlingspunkter],
    (behandlingspunktAksjonspunkter, overriddenBehandlingspunkter) =>
      arrayToObject(
        Object.keys(behandlingspunktAksjonspunkter),
        key => key,
        key => (overriddenBehandlingspunkter.includes(key) ? true : findApStatus(behandlingspunktAksjonspunkter[key])),
      ),
  );

  const hasInactiveAksjonspunkterSelector = createSelector(
    [getSelectedBehandlingspunktAksjonspunkter, getSelectedBehandlingspunktVilkar],
    aksjonspunkter => aksjonspunkter.some(ap => !ap.erAktivt),
  );

  const isSelectedBehandlingspunktReadOnly = createSelector(
    [getRettigheter, getBehandlingIsOnHold, hasReadOnlyBehandling, hasInactiveAksjonspunkterSelector],
    (rettigheter, behandlingIsOnHold, isBehandlingReadOnly, hasInactiveAksjonspunkter) => {
      return (
        !rettigheter.writeAccess.isEnabled || behandlingIsOnHold || isBehandlingReadOnly || hasInactiveAksjonspunkter
      );
    },
  );

  const isSelectedBehandlingspunktOverrideReadOnly = createSelector(
    [getRettigheter, hasInactiveAksjonspunkterSelector, isBehandlingStatusReadOnly],
    (rettigheter, hasInactiveAksjonspunkter, isStatusReadOnly) =>
      !rettigheter.kanOverstyreAccess.isEnabled || isStatusReadOnly || hasInactiveAksjonspunkter,
  );
  ...
  const isBehandlingspunktAksjonspunkterSolvable = createSelector(
    [getSelectedBehandlingspunktAksjonspunkter],
    (aksjonspunkter = []) => aksjonspunkter.some(ap => ap.kanLoses),
  );

  const getBehandlingspunktAksjonspunkterCodes = createSelector(
    [getSelectedBehandlingspunktAksjonspunkter],
    (aksjonspunkter = []) => aksjonspunkter.map(ap => ap.definisjon.kode),
  );
  ...
  return {
    ...
    hasInactiveAksjonspunkterSelector,
