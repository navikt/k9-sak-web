 ...
 */
const ankeBuilders = [
  new ProsessStegProperties.Builder(bpc.ANKEBEHANDLING, 'Ankebehandling').withAksjonspunktCodes(
    ac.MANUELL_VURDERING_AV_ANKE,
  ),
  new ProsessStegProperties.Builder(bpc.ANKE_RESULTAT, 'AnkeResultat').withAksjonspunktCodes(
    ac.FORESLA_VEDTAK,
    ac.FATTER_VEDTAK,
...
const createAnkeStegProps = builderData =>
  ankeBuilders.reduce((currentFbs, fb) => {
