...
describe('<AvviksOpplysningerATFLSN>', () => {
  it('Skal teste tabellen får korrekte rader for frilanser og kombinasjonsstatus', () => {
    ...
    expect(
      omregnetAarsinntektVerdi
        .at(1)
        .childAt(0)
        .text(),
    ).to.equal(formatCurrencyNoKr(beregnetAarsinntekt));
    ...
    expect(
      rapportertAarsinntektVerdi
        .at(1)
        .childAt(0)
        .text(),
    ).to.equal(formatCurrencyNoKr(sammenligningsgrunnlagSum));
    ...
    expect(
      avvikVerdi
        ...
        .childAt(0)
        .text(),
    ).to.equal(formatCurrencyNoKr(sammenligningsgrunnlagPrStatus.differanseBeregnet));
    const avvikProsentText = rows
      .at(3)
