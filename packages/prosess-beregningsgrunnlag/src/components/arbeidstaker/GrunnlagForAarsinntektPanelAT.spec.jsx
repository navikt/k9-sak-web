    ...
    andeler.forEach(andel => {
      ...
      expect(
        teksterAndel
          ...
          .childAt(0)
          .text(),
      ).to.equal(createVisningsnavnForAktivitet(andel.arbeidsforhold, getKodeverknavn));
      expect(
        teksterAndel
          .at(1)
          .childAt(0)
          .text(),
      ).to.equal(formatCurrencyNoKr(andel.beregnetPrAar / 12));
      expect(
        teksterAndel
          .at(2)
          .childAt(0)
          .text(),
      ).to.equal(formatCurrencyNoKr(andel.beregnetPrAar));

      const teksterArbForholdTid = rowsFlex.at(rowNrFlex).find('Undertekst');
      expect(
        teksterArbForholdTid
          .at(0)
          .childAt(0)
          .text(),
      ).to.equal(`${dateFormat(andel.arbeidsforhold.startdato)} - ${dateFormat(andel.arbeidsforhold.opphoersdato)}`);
      rowNr += 1;
      rowNrFlex += 1;
    });
  });
