export const getEndCharFromId = (id, substringLength = 4) => (id ? `...${id.substring(id.length - substringLength, id.length)}` : '');
...
export const finnEksternIdSubstring = (arbeidsgiverIdent, internId, arbeidsgiverOpplysningerPerId) => {
    ...
    const referanseForInternId = arbeidsforholdreferanser.find(referanse =>
        referanse.internArbeidsforholdId === internId);
    ...
    let referanserMedLikeSisteSiffer = arbeidsforholdreferanser.filter(({eksternArbeidsforholdId}) => getEndCharFromId(eksternArbeidsforholdId) === sisteSiffer);
    let i = 4;
    while (referanserMedLikeSisteSiffer.length > 1) {
        i += 1;
        sisteSiffer = getEndCharFromId(eksternId, i);
        referanserMedLikeSisteSiffer = arbeidsforholdreferanser
            .filter(({eksternArbeidsforholdId}) => eksternArbeidsforholdId.length >= i && getEndCharFromId(eksternArbeidsforholdId, i) === sisteSiffer);
    }
    return sisteSiffer;
