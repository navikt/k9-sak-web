...
const isSelvstendigOrFrilanser = fieldVal =>
  isSelvstendigNæringsdrivende(fieldVal.inntektskategori) || inntektskategorier.FRILANSER === fieldVal.inntektskategori;

const renderMessage = (intl, error) => (error[0] && error[0].id ? intl.formatMessage(...error) : error);

const getErrorMessage = (meta, intl) => (meta.error && meta.submitFailed ? renderMessage(intl, meta.error) : null);
...
const arbeidsforholdReadOnlyOrSelect = (fields, index, elementFieldId, selectVals, isReadOnly, arbeidsforholdList) => (
  <>
    {!fields.get(index).nyAndel && <InputField name={`${elementFieldId}.andel`} bredde="L" readOnly />}
...
const skalViseSletteknapp = (index, fields, readOnly) =>
  (fields.get(index).nyAndel || fields.get(index).lagtTilAvSaksbehandler) && !readOnly;
...
RenderFordelBGFieldArray.validate = (
  ...
  if (fastsattForUgraderteAktiviteterError) {
    return {
      _error: (
        <FormattedMessage
          id={fastsattForUgraderteAktiviteterError[0].id}
          values={fastsattForUgraderteAktiviteterError[1]}
        />
      ),
    };
  }
