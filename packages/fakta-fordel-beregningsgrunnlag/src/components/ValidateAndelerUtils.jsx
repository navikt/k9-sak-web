...
import { dateIsAfter, formatCurrencyNoKr, removeSpacesFromNumber, required } from '@fpsak-frontend/utils';
import {
  erAAPEllerArbeidsgiverOgSkalFlytteMellomAAPOgArbeidsgiver,
  GRADERING_RANGE_DENOMINATOR,
  mapToBelop,
...
const erAndelerLike = (andel1, andel2) =>
  andel2.andelsinfo === andel1.andelsinfo && andel2.inntektskategori === andel1.inntektskategori;
...
export const tomErrorMessage = () => [{ id: ' ' }];

export const totalRefusjonMåVereLavereEnn = seksG => [
  { id: 'BeregningInfoPanel.FordelBG.Validation.TotalRefusjonSkalIkkeOverstige' },
  { seksG },
];

const totalRefusjonSkalVereLavereEnn = (value, seksG) =>
...
export const totalFordelingForArbeidstakerMåVereLavereEnn = seksG => [
  { id: 'BeregningInfoPanel.FordelBG.Validation.TotalFordelingForArbeidstakerLavereEnn' },
  { seksG },
];

export const totalFordelingForArbeidstakerOgFrilanserMåVereLavereEnn = seksG => [
  { id: 'BeregningInfoPanel.FordelBG.Validation.TotalFordelingForArbeidstakerOgFrilanserLavereEnn' },
  { seksG },
];

export const totalFordelingForFrilanserMåVereLavereEnn = seksG => [
  { id: 'BeregningInfoPanel.FordelBG.Validation.TotalFordelingForFrilanserLavereEnn' },
  { seksG },
];

const totalFordelingSkalVereLavereEnn = (value, seksG, errorMessage) =>
...
export const validateFastsattBelopEqualOrBelowBeregningsgrunnlagPrAar = (fastsattBelop, beregningsgrunnlagPrAar) => {
  if (beregningsgrunnlagPrAar !== null && beregningsgrunnlagPrAar !== undefined) {
    return skalIkkjeVereHogareEnn(Number(fastsattBelop), beregningsgrunnlagPrAar, tomErrorMessage);
  }
  return null;
};
...
export const validateFastsattBelopEqualOrBelowRefusjon = (fastsattBelop, refusjon) => {
  if (refusjon !== null && refusjon !== undefined) {
    return skalIkkjeVereHogareEnn(Number(fastsattBelop), refusjon, tomErrorMessage);
  }
  return null;
};

export const validateAgainstBeregningsgrunnlag = (
  andelFieldValues,
  ...
  if (arbeidsforholdBelopValues) {
    ...
    if (skalValidereMotBeregningsgrunnlagPrAar(andelFieldValues)) {
      return validateFastsattBelopEqualOrBelowBeregningsgrunnlagPrAar(
        arbeidsforholdTotalFastsatt,
        arbeidsforholdBelopValues.beregningsgrunnlagPrAar,
      );
    }
  }
  return null;
...
export const validateFastsattBelop = (
  andelFieldValues,
  totalInntektArbeidsforholdList,
  skalValidereMotBeregningsgrunnlagPrAar,
  getKodeverknavn,
...
export const hasFieldErrors = fieldErrors =>
  fieldErrors.refusjonskrav || fieldErrors.andel || fieldErrors.fastsattBelop || fieldErrors.inntektskategori;

export const validateAndelFields = (
  andelFieldValues,
  totalInntektArbeidsforholdList,
  skalValidereMotBeregningsgrunnlagPrAar,
  getKodeverknavn,
...
export const validateAndeler = (values, skalValidereMotBeregningsgrunnlagPrAar, getKodeverknavn, periodeDato) => {
  ...
  const skalValidereMellomAAPOgArbeidsgiver = andel =>
    erAAPEllerArbeidsgiverOgSkalFlytteMellomAAPOgArbeidsgiver(andel, values);
  const totalInntektPrArbeidsforhold = lagTotalInntektArbeidsforholdList(
    values,
    skalValidereMotBeregningsgrunnlagPrAar,
  ...
  if (arrayErrors.some(errors => errors !== null)) {
    if (
      arrayErrors.some(
        errors => errors && errors.fastsattBelop && errors.fastsattBelop[0].id === tomErrorMessage()[0].id,
      )
    ) {
      return {
        ...arrayErrors,
...
export const validateSumFastsattBelop = (values, fordeling) => {
  const sumFastsattBelop = values.map(mapToBelop).reduce((sum, fastsattBelop) => sum + fastsattBelop, 0);
...
export const validateSumRefusjon = (values, grunnbeløp) => {
  const harGraderingUtenRefusjon = !!values.find(
    v => v.andelIArbeid !== '0.00' && convertToNumber(v.refusjonskrav) === 0,
  );
  const sumRefusjon = values
    .map(({ refusjonskrav }) => convertToNumber(refusjonskrav))
...
export const validateSumFastsattArbeidstaker = (values, seksG) => {
  const sumFastsattBelop = values
    .filter(v => v.aktivitetStatus === AktivitetStatus.ARBEIDSTAKER)
    .map(mapToBelop)
...
export const validateSumFastsattArbeidstakerOgFrilanser = (values, seksG) => {
  const sumFastsattBelop = values
    .filter(v => v.aktivitetStatus === AktivitetStatus.ARBEIDSTAKER || v.aktivitetStatus === AktivitetStatus.FRILANSER)
    .map(mapToBelop)
    .reduce((sum, fastsattBelop) => sum + fastsattBelop, 0);
...
export const validateSumFastsattForUgraderteAktiviteter = (values, grunnbeløp) => {
  const skalGradereFL = !!values.find(
  ...
  const skalGradereSN = !!values.find(
