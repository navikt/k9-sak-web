...
import { hasValidText, maxLength, minLength, requiredIfNotPristine } from '@fpsak-frontend/utils';
      ...
      {erOverstyrt && (
        <FlexContainer>
          ...
          <FlexRow>
            <FlexColumn>
              {!overrideReadOnly && (
                <Hovedknapp mini spinner={isSubmitting} disabled={isSubmitting || !isSolvable || isPristine}>
                  {intl.formatMessage({ id: 'OverstyringPanel.ConfirmInformation' })}
                </Hovedknapp>
              )}
            </FlexColumn>
            <FlexColumn>
              <Knapp htmlType="button" spinner={isSubmitting} disabled={isSubmitting} onClick={toggleAv} mini>
