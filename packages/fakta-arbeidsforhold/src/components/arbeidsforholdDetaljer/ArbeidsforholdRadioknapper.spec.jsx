...
describe('<ArbeidsforholdRadioknapper>', () => {
  ...
  it('skal vise radioknapper når aktivt arbeidsforhold, uten IM, fom før stp, tom etter stp', () => {
    ...
    expect(radioOptions.get(1).props.label.id).to.eql(
      'PersonArbeidsforholdDetailForm.AvslaYtelseManglendeOpplysninger',
    );
    expect(radioOptions.get(1).props.disabled).to.eql(false);
    expect(radioOptions.get(2).props.label.id).to.eql(
      'PersonArbeidsforholdDetailForm.InntektIkkeMedIBeregningsgrunnlaget',
    );
    expect(radioOptions.get(2).props.disabled).to.eql(false);
    expect(radioOptions.get(3).props.label.id).to.eql(
      'PersonArbeidsforholdDetailForm.BenyttAInntektIBeregningsgrunnlag',
    );
  ...
  it('Skal vise utvidet RadioOptions for aktivt arbeidsforhold når arbeidsforholdet har permisjon uten tom og ikke mottatt IM', () => {
    ...
    const radioOptions = wrapper.find('RadioOption');
    expect(radioOptions).has.length(5);
    expect(radioOptions.get(0).props.label.id).to.eql(
  ...
  it(
    'Skal vise utvidet RadioOptions for aktivt arbeidsforhold når arbeidsforholdet har flere permisjoner uten mottatt IM, ' +
  ...
  it(
    'Skal vise utvidet RadioOptions for aktivt arbeidsforhold når arbeidsforholdet har flere permisjoner og mottatt IM, ' +
      'samt disable knapp hvor saksbehandler kan velge at soeker er i permisjon',
    () => {
      const wrapper = shallowWithIntl(
        <ArbeidsforholdRadioknapper
