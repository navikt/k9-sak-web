 ...
 */
const propsWithValue = (propNames, value) =>
  propNames.map(propName => ({ [propName]: value })).reduce((a, b) => ({ ...a, ...b }));
...
const requireProps = (requiredPropNames: string[], alternative?: ReactNode) => WrappedComponent => {
  class ComponentWithRequiredProps extends Component {
    ...
    missingRequirements() {
      const { props } = this;
      return requiredPropNames.map(key => props[key]).some(prop => prop === null || prop === undefined);
    }
