...
const { AUTOMATISK_MARKERING_AV_UTENLANDSSAK, MANUELL_MARKERING_AV_UTLAND_SAKSTYPE } = aksjonspunktCodes;
    ...
    <>
      ...
      {visEditeringsmodus && (
        <FlexContainer>
          <FlexRow>
            <FlexColumn>
              <RadioGroupField direction="vertical" name="utlandSakstype" bredde="M" validate={[required]}>
                <RadioOption label={{ id: 'UtlandPanel.Nasjonal' }} value={utlandSakstypeKode.NASJONAL} />
                <RadioOption label={{ id: 'UtlandPanel.EøsBosattNorge' }} value={utlandSakstypeKode.EØS_BOSATT_NORGE} />
                <RadioOption label={{ id: 'UtlandPanel.BosattUtland' }} value={utlandSakstypeKode.BOSATT_UTLAND} />
              </RadioGroupField>
            </FlexColumn>
          </FlexRow>
          <VerticalSpacer sixteenPx />
          <FlexRow>
            <FlexColumn>
              <Hovedknapp mini onClick={slaPaVisningAvModal} disabled={!dirty}>
                <FormattedMessage id="UtlandPanel.lagre" />
              </Hovedknapp>
            </FlexColumn>
            <Knapp htmlType="button" mini onClick={slaAvEditeringAvUtland}>
      ...
      <UtlandEndretModal visModal={visModal} lagreOgLukk={handleSubmit} />
    </>
  );
};
...
const mapStateToPropsFactory = (_initialState, initialOwnProps) => {
  ...
  return (state, ownProps) => ({
    ...
    utlandSakstype: behandlingFormValueSelector(
      'UtlandPanel',
      ownProps.behandlingId,
      ownProps.behandlingVersjon,
    )(state, 'utlandSakstype'),
    onSubmit,
  });
};
