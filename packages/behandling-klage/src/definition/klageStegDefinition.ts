...
import { Behandling, Vilkar, Aksjonspunkt } from '@k9-sak-web/types';
...
const getVedtakStatus = ({ behandlingsresultat }, bpAksjonspunkter) => {
 ...
 */
const klageBuilders = [
  new ProsessStegProperties.Builder(
    bpc.FORMKRAV_KLAGE_NAV_FAMILIE_OG_PENSJON,
    'FormkravKlageNFP',
  ).withAksjonspunktCodes(ac.VURDERING_AV_FORMKRAV_KLAGE_NFP),
  new ProsessStegProperties.Builder(bpc.KLAGE_NAV_FAMILIE_OG_PENSJON, 'CheckKlageNFP').withAksjonspunktCodes(
    ac.BEHANDLE_KLAGE_NFP,
  ),
  new ProsessStegProperties.Builder(bpc.FORMKRAV_KLAGE_NAV_KLAGEINSTANS, 'FormkravKlageKA').withAksjonspunktCodes(
    ac.VURDERING_AV_FORMKRAV_KLAGE_KA,
  ),
  new ProsessStegProperties.Builder(bpc.KLAGE_NAV_KLAGEINSTANS, 'CheckKlageNK').withAksjonspunktCodes(
    ac.BEHANDLE_KLAGE_NK,
  ),
...
const createKlageStegProps = builderData =>
  klageBuilders.reduce((currentFbs, fb) => {
