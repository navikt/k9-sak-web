...
envsubst '$APP_URL $APP_PORT $APP_HOSTNAME $APP_NAME $APP_VERSION $APP_PATH_PREFIX $APP_URL_K9FORMIDLING $APP_URL_K9FORMIDLING_DD $APP_URL_K9OPPDRAG $APP_URL_KLAGE $APP_URL_K9TILBAKE $APP_URL_K9FORDEL $MEDISINSK_VILKAR_FRONTEND_URL $OMSORGSDAGER_FRONTEND_URL $PSB_UTTAK_FRONTEND_URL $OMSORGEN_FOR_FRONTEND_URL $PSB_INNTEKTSMELDING_FRONTEND_URL $PSB_ETABLERT_TILSYN_FRONTEND_URL $PSB_OM_BARNET_FRONTEND_URL $ENDRINGSLOGG_URL' < /etc/nginx/conf.d/app.conf.template > /etc/nginx/conf.d/default.conf
